...
func _physics_process(delta: float) -> void:
	if is_colliding():
		...
		var spring_length = clamp(distance - wheel_radius, 0, vehicle.SUSPENSION_REST_DISTANCE)
		var spring_force = vehicle.SUSPENSION_SPRING_STRENGTH * (vehicle.SUSPENSION_REST_DISTANCE - spring_length)
		var spring_velocity = (prev_spring_length - spring_length) / delta
		var damper_force = vehicle.SUSPENSION_SPRING_DAMPER * spring_velocity
