...
const CAMERA_SENSIVITY: float = 2.5
...
func _physics_process(delta: float) -> void:
	...
	if not isPaused:
		...
		camera.rotation.x = clamp(camera.rotation.x - look_dir.y * CAMERA_SENSIVITY * delta,
								  deg_to_rad(-90), deg_to_rad(90))
		look_dir = Vector2.ZERO

		# Get the input direction and handle the movement/deceleration.
		var playerSpeed := (2 * SPEED) if Input.is_action_pressed("Sprint") else SPEED
